...
mysql_select_db('autoescola') or trigger_error(mysql_error());

$usuario = mysql_real_escape_string($_POST['usuario']);
$senha = mysql_real_escape_string($_POST['senha']);

// Validaчуo do usuсrio/senha digitados
$sql = "SELECT `id_login`, `nome`, `nivel` FROM `login` WHERE (`usuario` = '". $usuario ."') AND (`senha` = '". md5($senha) ."') AND (`ativo` = 1) LIMIT 1";
$query = mysql_query($sql);
if (mysql_num_rows($query) != 1) {
	// Mensagem de erro quando os dados sуo invсlidos e/ou o usuсrio nуo foi encontrado, redireciona o visitante.
	header("Location: errologin.php"); exit;
